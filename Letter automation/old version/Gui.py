...
def select_template(gen_string):
    ...
    return templates.get(gen_string, {})

def select_pdf(gen_string):
    pdf_mapping = {
        "letter/public": "Sample-Fillable-PDF",
        "message/private": "MessagePrivate.pdf",
        # ... add more mappings as needed
    }
    return pdf_mapping.get(gen_string, None)


class PDFSection(ttk.LabelFrame):
    def __init__(self, parent, template_fields, fill_pdf_callback):
        super().__init__(parent, text="2nd Section", padding=(10, 5))
        self.field_widgets = {}  # Dictionary to store references to combobox widgets
        self.fill_pdf_callback = fill_pdf_callback
    ...
    def add_continue_button(self):
        ttk.Button(self, text="Continue", command=self.fill_pdf_callback).grid(row=len(self.field_widgets)+1, column=0, columnspan=2, pady=20)
...
class LetterAutomationGUI(tk.Tk):
    ...
    def print_string(self):
        ...
        template_fields = select_template(self.generated_string)
    ...
    def show_pdf_section(self, template_fields):
        ...
        self.pdf_section = PDFSection(self, template_fields, self.fill_pdf)
        self.pdf_section.pack(padx=20, pady=20, fill=tk.BOTH, expand=True)
        
    def fill_pdf(self):
        # Get the corresponding PDF
        pdf_file = select_pdf(self.generated_string)
        if pdf_file == "Sample-Fillable-PDF":
            ...
            data_dict = {
                'Name': self.pdf_section.field_widgets["Name"].get(),
                'Dropdown2': 'Choice 1',
                'Option 1': '',
                'Option 2': '',
                'Option 3': '',
                'Name of Dependent': self.pdf_section.field_widgets["Dropdown2"].get(),
                'Age\t of Dependent': ''
            }
